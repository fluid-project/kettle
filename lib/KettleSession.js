/**
 * Kettle Session support, using standard connect session middleware
 *
 ...
 */
kettle.use.session.useSession = function (handlerOrRequest) {
...
kettle.use.session.checkHandlerSession = function (that, handler) {
    if (!kettle.use.session.useSession(handler)) {
        return;
    }
    if (!that.cookieParser) {
        fluid.fail("Sessions requires cookieParser middleware.");
    }
    if (!that.session) {
        fluid.fail("Sessions requires session middleware.");
    }
};
 ...
 */
kettle.sessionManager.createSession = function (that, request) {
    // If session already exists or needs to be created proceed to session middleware.
    if (kettle.use.session.useSession(request)) {
...
kettle.sessionManager.makeStore = function () {
    return new kettle.connect.MemoryStore();
};

fluid.defaults("kettle.sessionManager.requestSession", {
    gradeNames: ["fluid.component", "{that}.getSessionTypeGrade"],
    mergePolicy: {
...
fluid.defaults("kettle.sessionManager.requestSession.http", {
    gradeNames: ["kettle.sessionManager.requestSession"],
    listeners: {
...
kettle.sessionManager.requestSession.destroySession = function (session, afterDestroySession) {
    var wrappedAfterDestroySession = kettle.wrapCallback(afterDestroySession.fire);
    session.destroy(wrappedAfterDestroySession);
};
