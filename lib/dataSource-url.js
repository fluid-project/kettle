...
kettle.dataSource.URL.requestOptions = [
    ...
    "url", "protocol", "host", "hostname", "family", "port", "localAddress",
    "socketPath", "method", "path", "headers", "auth", "agent", "termMap",
...
kettle.dataSource.URL.handle.http = function (that, baseOptions, data) {
    ...
    if (requestOptions.protocol === "http:") {
        req = kettle.npm.http.request(requestOptions, callback);
    } else if (requestOptions.protocol === "https:") {
        req = kettle.npm.https.request(requestOptions, callback);
    } else {
