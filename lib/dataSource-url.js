 ...
 * @param toCensor {Object} A hash of member names of `requestOptions` to `true`, holding those which should be censored
 * @return A requestOptions object with the sensitive members removed where they appear at top level as well as encoded
 * within the `url` member
 */

kettle.dataSource.prepareLoggingOptions = function (requestOptions, toCensor) {
    var sensitiveValues = fluid.values(fluid.transform(toCensor, function (value, key) {
        return value ? requestOptions[key] : null;
    }));
    sensitiveValues.push("url");
    var censored = fluid.censorKeys(requestOptions, fluid.keys(toCensor));
    censored.url = urlModule.format(censored);
    return censored;
};
...
kettle.dataSource.URL.handle.http = function (that, baseOptions, data) {
    ...
    var loggingOptions = kettle.dataSource.prepareLoggingOptions(requestOptions, that.options.censorRequestOptionsLog);
