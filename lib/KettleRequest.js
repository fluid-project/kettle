...
kettle.markActiveRequest = function (request) {
    ...
    if (request && innerRequest && innerRequest !== request) {
        console.log("kettle.markActiveRequest found stale request ", fluid.prettyPrintJSON(innerRequest), " about to fail");
    ...
    if (request) {
        if (innerRequest !== request) {
            fluid.log("Marking request " + request.id + " active");
        ...
        if (innerRequest) { // unmarked automatically if destroyed
            fluid.log("Marking request " + innerRequest.id + " inactive");
...
kettle.request.initiateHandleRequest = function (request, rootSequence) {
    fluid.log("Kettle initiateHandleRequest allocated request component with type " + request.typeName + " and id " + request.id);
    ...
    fluid.log("Kettle initiateHandleRequest has concluded for request id " + request.id);
};
...
fluid.defaults("kettle.request.http", {
    ...
    listeners: {
        ...
        "onRequestError.log": {
            funcName: "kettle.request.http.logError",
...
kettle.request.http.logError = function (request, error) {
    fluid.log("Request id " + request.id + " has received error ", error);
};
