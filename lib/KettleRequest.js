...
kettle.markActiveRequest = function (request) {
    ...
    if (request && innerRequest && innerRequest !== request) {
        fluid.fail("Error marking threat to request " + request.id + " - this thread is already marked to request " + innerRequest.id + " . Make sure to invoke this request asynchronously.");
    }
    if (request) {
