...
kettle.request.handleRequest = function (that) {
    try {
        ...
        console.log("EXCEPTION IN HANDLEREQUEST: ", err);
        if (!that.handlerPromise.disposition) {
            that.handlerPromise.reject(err);
        }
    } finally {
        console.log("END OF REQUEST " + that.id);
        kettle.markActiveRequest(null);
    }
};
...
kettle.request.http.handleFullRequest = function (request, fullRequestPromise, next) {
    fullRequestPromise.then(function (response) {
        console.log("RESOLVING OVERALL REQUEST with response ", response);
...
kettle.request.http.errorHandler = function (res, error) {
    ...
    console.log("KETTLE REQUEST ERROR SENDING ", outError);
    res.status(outError.statusCode).send(fluid.censorKeys(outError, ["statusCode"]));
    return outError;
};
...
kettle.request.http.successHandler = function (request, response) {
    console.log("KETTLE REQUEST SUCCESS SENDING ", response + " for method " + request.req.method);
