...
kettle.dataSource.file.handle = function (that, requestOptions, directModel, model) {
    ...
    promise.accumulateRejectionReason = function (originError) {
        return kettle.dataSource.upgradeError(originError, " while " + (operation === "set" ? "writing" : "reading") + " file " + fileName);
    };
    if (operation === "set") {
        method = "writeFile";
...
kettle.dataSource.URL.relayError = function (response, received, whileMsg) {
    ...
    return kettle.dataSource.upgradeError(rejectPayload, whileMsg);
};
...
kettle.dataSource.URL.errorCallback = function (promise, whileMsg) {
    return function (error) {
        promise.reject(kettle.dataSource.upgradeError(error, whileMsg));
    };
};
...
kettle.dataSource.URL.handle.http = function (that, baseOptions, data) {
    ...
    promise.accumulateRejectionReason = function (originError) {
        return kettle.dataSource.upgradeError(originError, whileMsg);
    };

    var req = http.request(requestOptions, kettle.wrapCallback(kettle.dataSource.URL.httpCallback(requestOptions, promise, whileMsg)));
