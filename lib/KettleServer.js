...
fluid.defaults("kettle.server", {
    ...
    rootMiddleware: { // free hash of middleware names to components - these will be applied at top level via app.use on startup
        urlencoded: {
...
kettle.server.getHandlerSequence = function (that) {
    var resolved = kettle.middleware.resolve(that, "Kettle server rootMiddleware entry", that.options.rootMiddleware);
