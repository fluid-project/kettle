...
kettle.server.sendMismatchError = function (error, res) {
    console.log("Sending MISMATCHED PROTOCOL ERROR " + error);
    return kettle.request.http.errorHandler(res, error);
};
...
kettle.server.evaluateRoute = function (server, req, originOptions) {
    ...
    if (match) {
        ...
        fluid.log("Invoking handler " + handler.type + " for match ", fluid.filterKeys(match, ["app"]) , " with expectedGrade " + originOptions.expectedRequestGrade);
        var defaults = fluid.defaults(handler.type, handler.gradeNames);
        console.log("Got DEFAULTS with grades ", defaults.gradeNames);
        match.mismatchError = kettle.server.checkCompatibleRequest(originOptions.expectedRequestGrade, defaults.gradeNames);
    }
    return match;
};
...
kettle.server.checkCreateRequest = function (server, req, res, next, originOptions) {
    ...
    if (match) {
        if (match.mismatchError) {
            return kettle.server.sendMismatchError(match.mismatchError, res);
        }
        $.extend(req, match.output);
        var handler = match.handler;
        var options = $.extend({gradeNames: handler.gradeNames}, originOptions);
...
kettle.server.getDispatcher = function (that, rootSequence) {
    return function (req, res, next, options) {
        ...
        if (fluid.isComponent(request)) {
            console.log("ALLOCATED REQUEST with type ", request.typeName);
            ...
            handleRequestPromise.then(next, function (err) {
                // TODO: make this policy configurable
                request.events.onError.fire(err);
                console.log("REJECTING OVERALL REQUEST with err ", err);
                next(err);
            }); // we specify that a resolve resolves nothing
        } else if (request && request.isError) {
            console.log("DISPATCHER SENDING ERROR ", request);
            next(request);
        } else {
