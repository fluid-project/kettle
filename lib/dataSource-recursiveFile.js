...
kettle.dataSource.recursiveFile.findAll = function (rootPath, toFind, found, callback) {
    fs.readdir(rootPath, function (err, files) {
        ...
        (function next() {
            ...
            var currentFullPath = path.join(rootPath, currentFile);

            i++;

            fs.lstat(currentFullPath, function (err, stats) {
                if (err) {
                    callback(err.message, toFind);
                } else if (stats && stats.isDirectory()) { // get stats
                    kettle.dataSource.recursiveFile.findAll(currentFullPath, toFind, found, function () {
                        pending--;
                        (pending === 0) ? callback() : next();
                    });
                } else {
                    if (currentFile === toFind && stats && stats.isFile()) { // if this is the file we're looking for push to list
                        found.push(currentFullPath);
                    }
                    pending--;
                    (pending === 0) ? callback() : next();
                }
