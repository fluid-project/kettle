...
(function () {
    ...
    var loadModules = function (config, prefix) {
        var modoules = config.modules;
        if (!modules || modules.length < 1) {
            return;
        }
        var moduleLoader = config.moduleLoader;
        if (!moduleLoader) {
            fluid.log("No module loader provided");
            return;
        }
