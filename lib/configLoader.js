...
(function () {
    ...
    var loadModules = function (modules, prefix) {
        if (!modules || modules.length < 1) {
            return;
        }
        var moduleLoaderFileName = "__kettleModuleLoader.js";
        var moduleLoaderFullPath = path.resolve(prefix, moduleLoaderFileName);
        var data = fs.readFileSync(
            path.resolve(__dirname, moduleLoaderFileName));
        fs.writeFileSync(moduleLoaderFullPath, data);
        var moduleLoader = require(moduleLoaderFullPath);
        fluid.each(modules, function loadModule(module) {
            fluid.require(module, moduleLoader);
        });
        fs.unlinkSync(moduleLoaderFullPath);
    };
    ...
    var loadConfigImpl = function (config, prefix, filePath) {
        ...
        loadModules(configFile.modules, prefix);
