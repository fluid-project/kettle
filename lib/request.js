...
(function () {
    ...
    fluid.defaults("kettle.requests.request", {
        ...
        invokers: {
            ...
            getSessionGrade: {
                funcName: "kettle.requests.request.getSessionGrade",
                args: "{kettle.server}.options.gradeNames"
            }
        },
        components: {
            handler: {
    ...
    /**
     * Determine if we need to apply a dynamic request session grade that adds
     * session support.
     * @param  {Array} gradeNames kettle.server's grades.
     * @return {String} a request session grade.
     */
    kettle.requests.request.getSessionGrade = function (gradeNames) {
        if (gradeNames.indexOf("kettle.use.session") >= 0) {
            return "kettle.requests.request.session";
        }
    };

    fluid.defaults("kettle.requests.request.session", {
        gradeNames: ["autoInit", "fluid.eventedComponent"],
        events: {
        ...
        dynamicComponents: {
            session: {
                ...
                type: "kettle.requests.requestSession",
                options: {
                    session: "{arguments}.0"
                }
            }
    ...
    fluid.defaults("kettle.requests.requestSession", {
        gradeNames: ["autoInit", "fluid.eventedComponent"],
        mergePolicy: {
        ...
        invokers: {
            destroySession: {
                funcName: "kettle.requests.requestSession.destroySession",
     ...
     * destroyed.
     */
    kettle.requests.requestSession.destroySession = function (session, afterDestroySession) {
        session.destroy(afterDestroySession.fire);
    };

    fluid.defaults("kettle.requests.request.http", {
        mergePolicy: {
            "req": "preserve, nomerge",
            "res": "preserve, nomerge",
            "next": "preserve, nomerge"
        },
        members: {
