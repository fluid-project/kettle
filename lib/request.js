...
(function () {
    ...
    kettle.requests.request.deferred.when = function (evented, promiseOrValue, callback) {
        // All the callbacks supplied to when should be wrapped, since versions after 2.0.0 will resolve asynchronously
        // Wrapping them for earlier versions is harmless
        return when(promiseOrValue, kettle.wrapCallback(function onSuccess(resolved) {
            evented.events.onSuccess.fire(callback ? callback(resolved) : resolved);
        }), kettle.wrapCallback(function onError(error) {
