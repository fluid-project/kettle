...
/** Upgrades a promise rejection payload by suffixing an additional "while" reason into its "message" field
 * @param error {Object} A rejection payload. This should (at least) have the member `isError: true` set, as well as a String `message` holding a rejection reason.
 * @return {Object} The rejected payload formed by shallow cloning the supplied argument and suffixing its `message` member
 */
kettle.dataSource.upgradeError = function (originError, whileMsg) {
