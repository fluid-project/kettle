...
(function () {
     ...
     * @return {String} a short form tranfromation from context to result
     * grades. For example 'grade1,grade2=>grade3'
     */
    kettle.linkage.indexer = function (defaults) {
        if (!defaults.contextGrades || !defaults.resultGrades) {
            return;
        }
        return [fluid.makeArray(defaults.contextGrades).join(",") + "=>" +
            fluid.makeArray(defaults.resultGrades).join(",")];
    };
    ...
    kettle.getDynamicGrade = function (gradeNames) {
        ...
        for (var linkage in linkages) {
            var segs = linkage.split("=>");
            var contextGrades = fluid.makeArray(segs[0].split(","));
            var resultGrades = fluid.makeArray(segs[1].split(","));
            var exclude = fluid.find(contextGrades, function (grade) {
                if (gradeNames.indexOf(grade) < 0) {
                    return true;
                }
            });
            if (!exclude) {
                togo.push.apply(togo, resultGrades);
            }
        }
        return togo;
    };
