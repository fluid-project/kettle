...
(function () {
    ...
    kettle.middleware.cookieParser.makeParser = function () {
        return kettle.utils.cookieParser();
    };
    ...
    fluid.defaults("kettle.middleware.sessionValidator", {
        ...
        invokers: {
            ...
            handle: {
                ...
                args: [
                    "{that}.validate",
                    "{kettle.middleware.session}.options.key",
                    "{arguments}.0"
                ]
            }
        }
     ...
     */
    kettle.middleware.sessionValidatorHandle = function (validate, key, request) {
        ...
        if (request.needValidSession && !validate(request)) {
            ...
            var res = request.res;
            request.req.session.destroy(function onSessionDestroy() {
                res.clearCookie(key);
                res.send(403, {
                    isError: true,
    ...
    kettle.middleware.sessionHandle = function (session, routes, handlers, request) {
        ...
        // None of the hanlders handle the path of the request.
        if (!route) {
            res.send(500, {
                isError: true,
    ...
    /**
     * A grade that provides all necessary support for sessions wihtin the
