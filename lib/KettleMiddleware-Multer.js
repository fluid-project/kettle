...
fluid.defaults("kettle.middleware.multer.storage.disk", {
    ...
    members: {
        // Required configuration object structure for Multer's internal
        // diskStorage implementation
        //
        // TODO: is this the best way to be able to supply an object to an
        // invoker where the object uses IoC references for the values of its
        // keys?
        diskStorageConfigurationObject: {
            destination: "{that}.destinationImpl",
            filename: "{that}.filenameImpl"
        }
    },
    invokers: {
        // Invokers overriding the "destinationImpl" and "filenameImpl" functions should
        ...
        destinationImpl: {
            funcName: "kettle.middleware.multer.storage.disk.destinationImpl",
            args: ["{arguments}.0", "{arguments}.1", "{that}.options.destination", "{arguments}.2"]
        },
        filenameImpl: {
            funcName: "kettle.middleware.multer.storage.disk.filenameImpl",
...
kettle.middleware.multer.storage.disk.destinationImpl = function (req, file, destinationTemplate, cb) {
    var destination = fluid.stringTemplate(destinationTemplate, file);
    cb(null, destination);
};

kettle.middleware.multer.storage.disk.filenameImpl = function (req, file, filenameTemplate, cb) {
