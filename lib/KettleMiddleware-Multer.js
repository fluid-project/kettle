...
fluid.defaults("kettle.middleware.multer.storage.disk", {
    ...
    invokers: {
        // Invokers overriding the "destination" and "filename" functions should
        // use argument-shifting as below to supply a function to Multer with its
        // expected signature of req, file, cb
        // See https://github.com/expressjs/multer#diskstorage for more
        // details
        destination: {
...
kettle.middleware.multer.storage.disk.destination = function (req, file, destinationTemplate, cb) {
        cb(null, fluid.stringTemplate(destinationTemplate, file));
};

kettle.middleware.multer.storage.disk.filename = function (req, file, filenameTemplate, cb) {
