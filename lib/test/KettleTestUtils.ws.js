...
fluid.defaults("kettle.test.request.ws", {
    ...
    events: {
        ...
        onClose: null // fires when socket is closed
    },
    listeners: {
...
kettle.test.request.ws.requestOptions = ["protocol", "agent", "headers", "protocolVersion", "hostname", "port", "pathname", "termMap"];
...
kettle.test.request.ws.connect = function (that, cookieJar, directOptions) {
    ...
    directOptions.protocol = "ws";
    var requestOptions = kettle.test.request.http.prepareRequestOptions(that.options, cookieJar, directOptions, kettle.test.request.ws.requestOptions);

    fluid.log("connecting ws.WebSocket to: " + requestOptions.url + " with request options ", requestOptions);

    that.ws = new kettle.npm.ws(requestOptions.url, that.options.webSocketsProtocols, requestOptions);
    ...
    that.ws.on("error", function (err) {
        fluid.log("kettle.test.request.ws client error", err);
        that.events.onError.fire(that, err);
    });
    that.ws.on("message", function (data) {
        fluid.log("kettle.test.request.ws client message", data);
        that.events.onReceiveMessage.fire(that.options.receiveJSON ? kettle.JSON.parse(data) : data, that);
    });
    that.ws.on("close", function (code, reason) {
        fluid.log("kettle.test.request.ws closed, response code: ", code, ", reason: ",  reason);
        that.events.onClose.fire(that, {
            code: code,
            reason: reason
        });
    });
