...
fluid.defaults("kettle.test.request.formData", {
    ...
    formData: {
        ...
        // name and the value is the path to a file to be attached
        files: {
            // "file": "./LICENSE.txt"
        },
        fields: {
...
kettle.test.request.formData.send = function (that, callback) {
    ...
    var submitURL = "http://" + that.options.hostname + ":" + that.options.port + that.options.path;

    var form = new FormData();

    // Append all files
    fluid.each(that.options.formData.files, function (filePath, fieldKey) {
        form.append(fieldKey, fs.createReadStream(filePath));
    });

    // Append all fields
    fluid.each(that.options.formData.files, function (fieldValue, fieldKey) {
