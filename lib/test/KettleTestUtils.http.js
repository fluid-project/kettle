...
// TODO: This is copied out of fluid.dataSource.URL.handle - factor this better
...
kettle.test.request.http.prepareRequestOptions = function (options, cookieJar, directOptions, permittedOptions) {
    if (directOptions) {
        directOptions.pathname = directOptions.pathname || directOptions.path;
    }
    var resolved = fluid.dataSource.URL.resolveUrl(options.url, options.termMap);
    var parsed = resolved ? fluid.dataSource.URL.parse(resolved, permittedOptions) : {};
    var togo = fluid.dataSource.URL.prepareRequestOptions(options, cookieJar, directOptions, permittedOptions, {}, parsed);
    delete togo.path;
...
kettle.test.request.http.prepareRequest = function (that, optsObject) {
    ...
    var requestOptions = kettle.test.request.http.prepareRequestOptions(that.options, cookieJar, directOptions, kettle.dataSource.URL.requestOptions);

    fluid.log("Sending a " + (requestOptions.method || "GET") + " request to: ", requestOptions.url);

    var req = that.nativeRequest = http.request(requestOptions.url, requestOptions, function (res) {
        ...
        res.on("close", function (err) {
            if (err) {
                fluid.fail("Error making request to " + requestOptions.url + ": " + err.message);
            }
        });
    ...
    req.on("error", function (err) {
        if (that.options.failOnError) {
            jqUnit.fail("Error making request to " + requestOptions.url + ": " + err.message);
        } else {
