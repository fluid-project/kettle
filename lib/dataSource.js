...
(function () {
    ...
    fluid.defaults("kettle.dataSource", {
        ...
        invokers: {
            ...
            set: {
                expander: {
                    func: "kettle.dataSource.allowSet",
    ...
    kettle.dataSource.allowSet = function allowSet(writable) {
        if (writable) {
            return "kettle.dataSource.set";
        }
    };
    ...
    fluid.defaults("kettle.dataSource.modelParser", {
        gradeNames: ["autoInit", "fluid.littleComponent"],
        invokers: {
            stringify: "kettle.dataSource.modelParser.stringify",
            parse: "kettle.dataSource.modelParser.parse"
        }
    });

    fluid.demands("kettle.dataSource.modelParser.parse",
        "kettle.dataSource.modelParser", {
     ...
     */
    kettle.dataSource.modelParser.parse = function parse(responseParser, data) {
        if (!data) {
            return {
                isError: true,
    ...
    fluid.demands("kettle.dataSource.modelParser.stringify",
        "kettle.dataSource.modelParser", {
            args: "{arguments}.0"
        }
    );
     ...
     */
    kettle.dataSource.modelParser.stringify = function stringify(model) {
        if (!model) {
            return;
        }
    ...
    fluid.defaults("kettle.dataSource.errback", {
        ...
        invokers: {
            handleError: "kettle.dataSource.errback.handleError"
        }
    });

    fluid.demands("kettle.dataSource.errback.handleError",
        "kettle.dataSource.errback", {
            args: [
                "{callbackWrapper}",
                "{requestProxy}",
                "{arguments}.0"
     ...
     */
    kettle.dataSource.errback.handleError = function handleError(callbackWrapper, requestProxy, data) {
    ...
    fluid.defaults("kettle.dataSource.urlResolver", {
        ...
        invokers: {
            resolve: "kettle.dataSource.urlResolver.resolve"
        }
    });

    fluid.demands("kettle.dataSource.urlResolver.resolve",
        "kettle.dataSource.urlResolver", {
            args: [
                "{urlExpander}.expand",
     ...
     */
    kettle.dataSource.urlResolver.resolve = function resolve(expand, url, termMap, directModel) {
     ...
     */
    kettle.dataSource.CouchDB.wrapCallback = function wrapCallback(callback) {
    ...
    kettle.dataSource.CouchDB.finalInit = function finalInit(that) {
        var originalGet = that.get, originalSet;

        that.get = function get(directModel, callback) {
     ...
     * operation.
     */
    kettle.dataSource.URL.handle = function handle(options, directModel, callback, model) {
            // Expand URL.
    ...
    kettle.dataSource.URL.handle.url = function handleUrl(path, options, callback, model) {
    ...
    kettle.dataSource.URL.handle.fs = function handleFs(fileName, options, callback, model) {
        var method = "readFile",
     ...
     */
    kettle.dataSource.isFileProtocol = function isFileProtocol(uri) {
        return /^file:\/\//.test(uri);
    };

    fluid.defaults("kettle.callbackWrappingPromiseDataSource", {
        gradeNames: ["autoInit", "fluid.littleComponent"],
        components: {
            callbackWrapper: "{callbackWrapper}",
            rawSource: {
        ...
        finalInitFunction: "kettle.callbackWrappingPromiseDataSource.finalInit"
    });

    kettle.callbackWrappingPromiseDataSource.finalInit = function finalInit(that) {
        fluid.each(["get", "set"], function wrapMethod(method) {
            that[method] = function wrappedMethod() {
                ...
                args.push(that.callbackWrapper.wrap(function wrappedCallback() {
     ...
     */
    fluid.defaults("kettle.callbackWrappingDataSource", {
        gradeNames: ["autoInit", "fluid.littleComponent"],
        components: {
            callbackWrapper: "{callbackWrapper}",
            rawSource: {
        ...
        finalInitFunction: "kettle.callbackWrappingDataSource.finalInit"
    });

    kettle.callbackWrappingDataSource.finalInit = function finalInit(that) {
        fluid.each(["get", "set"], function wrapMethod(method) {
            that[method] = function wrappedMethod() {
