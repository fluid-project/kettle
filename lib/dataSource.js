 ...
 * entries in "termMap" which themselves take the form %path will be looked up within the argument `directModel`. All such entries will then be URI Encoded, unless their value
...
kettle.dataSource.renderJSONDiagnostic = function (string, promise) {
    ...
    } catch (e) {} // Cannot override the core exception throwing code within the shitty parser - at jsonlint.js line 157
    var errors = kettle.dataSource.JSONParseErrors.join("\n");
    promise.reject({
        isError: true,
        message: errors
    });
...
fluid.defaults("kettle.dataSource.errback", {
    ...
    listeners: { // Target this listener's namespace to customise error behaviour
        onError: {
