...
(function () {
    ...
    /**
     * Resolves (expands) a url or path with respect to the "directModel" supplied to a dataSource's API (get or set)
     * @param  {Function} expand a method of urlExpander that expands values within the url template.
    ...
    kettle.dataSource.defaultiseOptions = function (options, directModel, isSet) {
        options = options || {};
    ...
    kettle.dataSource.get = function (that, directModel, options) {
        options = kettle.dataSource.defaultiseOptions(options, directModel);
     ...
     * @param  {JSON} options an options block that contains operation: set/get
    ...
    kettle.dataSource.URL.handle.http = function (path, options, model) {
        ...
        if (options.operation === "set") {
            ...
            requestOptions.method = options.writeMethod || "PUT";
        }
        var req = http.request(requestOptions, function onRes(res) {
            var data = "";
    ...
    kettle.dataSource.URL.handle.fs = function (fileName, options, model) {
        var promise = fluid.promise(),
            method = "readFile",
            args = [fileName, "utf8", function onFsMethod(error, data) {
                if (error) {
                    ...
                    promise.resolve(options.operation === "get" ? data : undefined);
                }
            }];
        if (model) {
            method = "writeFile";
