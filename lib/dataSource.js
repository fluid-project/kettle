...
(function () {
    ...
    fluid.defaults("kettle.dataSource", {
        gradeNames: ["autoInit", "fluid.littleComponent", "{that}.getWritableGrade"],
        components: {
            errback: {
    ...
    kettle.dataSource.errback.handleError = function (requestProxy, data) {
        var fireOnError = kettle.wrapCallback(requestProxy.events.onError.fire);
        fireOnError(data);
    };
    ...
    kettle.dataSource.CouchDB.finalInit = function (that) {
        ...
        that.get = function (directModel, callback) {
            var couchDBCallback = kettle.dataSource.CouchDB.wrapCallback(callback);
    ...
    kettle.dataSource.URL.handle = function (options, directModel, callback, model) {
        ...
        options.fireOnError = kettle.wrapCallback(options.errback.events.onError.fire);
    ...
    kettle.dataSource.URL.handle.fs = function (fileName, options, callback, model) {
        // console.log("handle fs " + fileName);
        ...
        var method = "readFile",
            args = [fileName, "utf8", function onFsMethod(error, data) {
                ...
                // console.log("FS CALLBACK data ", data, " error ", error);
                callback(data || model);
            }];
        if (model) {
            method = "writeFile";
    ...
    // TODO: eliminate this stateful "finalInit" function in favour of a "dataSource calculus" that composes
    // immutable dataSources from a small declarative dialect
        ...
        fluid.each(["get", "set"], function wrapMethod(method) {
            ...
            that[method] = function wrappedMethod() {
                var deferred = when.defer(),
                    ...
                    wrap = kettle.wrapCallback;

                // We can not directly modify when's then method. It is set to
                // be non-writable (Throws TypeError if we try to assignment
                // operator).
                // TODO: This is an unstable scheme for ensuring callback wrapping. There are numerous other ways
                // to propagate callbacks into a promise.
                promiseImpl.then = function then(callback) {
                    promise.then(wrap(callback));
                };
     ...
     */
    fluid.defaults("kettle.dataSource.simpleCallbackWrapper", {
        gradeNames: ["autoInit", "fluid.littleComponent"]
    });

    kettle.dataSource.simpleCallbackWrapper.finalInit = function (that) {
        fluid.each(["get", "set"], function wrapMethod(method) {
            var originalMethod = that[method];
            if (!originalMethod) {
                return;
            ...
            that[method] = function wrappedMethod() {
                ...
                args[args.length - 1] = kettle.wrapCallback(callback);
                originalMethod.apply(null, args);
            };
        });
    };
