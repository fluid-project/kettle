...
kettle.dataSource.URL.requestOptions = ["protocol", "host", "hostname", "family", "port", "localAddress", "socketPath", "method", "path", "headers", "auth", "agent", "termMap"];


kettle.dataSource.URL.prepareRequestOptions = function (componentOptions, cookieJar, userOptions, permittedOptions) {
    ...
    requestOptions.path = fluid.stringTemplate(requestOptions.path, termMap);
...
kettle.dataSource.URL.handle = function (that, userOptions, directModel, model) {
        // Expand URL.
    var url = that.urlResolver.resolve(directModel),
        // Test whether the URL is to a local file.
        isFileProtocol = url.indexOf("file://") === 0,
        func = kettle.dataSource.URL.handle.http;
    if (isFileProtocol) {
        url = url.substring("file://".length);
        func = kettle.dataSource.URL.handle.fs;
    }
    return func.apply(null, [that, url, userOptions, model, that.cookieJar]);
};
...
kettle.dataSource.URL.handle.http = function (that, path, userOptions, data, cookieJar) {
    var promise = fluid.promise(),
        baseOptions = fluid.filterKeys(url.parse(path, true), kettle.dataSource.URL.requestOptions);
    baseOptions.port = baseOptions.port || 80;
    baseOptions.method = baseOptions.method || "GET";

    if (userOptions.operation === "set") {
        baseOptions.headers = {
            "Content-Type": "application/json",
            "Content-Length": data.length
        };
        baseOptions.method = userOptions.writeMethod;
    }
    fluid.extend(true, baseOptions, userOptions);
    var requestOptions = kettle.dataSource.URL.prepareRequestOptions(that.options, cookieJar, baseOptions, kettle.dataSource.URL.requestOptions);
...
kettle.dataSource.URL.handle.fs = function (that, fileName, requestOptions, data) {
    var promise = fluid.promise(),
        method = "readFile",
        ...
        fsCallback = function (error, data) {
            if (error) {
                ...
                promise.resolve(requestOptions.operation === "set" ? undefined : data);
            }
        },
        args = [fileName, "utf8"];
