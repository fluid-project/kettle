...
(function () {
    ...
    kettle.dataSource.modelParser.parse = function (responseParser, data) {
        // if (!data) { //TODO: DELETE THIS BLOCK
        //     return {
        //         isError: true,
    ...
    kettle.dataSource.CouchDB.wrapCallback = function (callback) {
        return function wrappedCallback(resp) {
            //if undefined, pass that through, else clean up fields:
            console.log("wrapped callback returning ===="+JSON.stringify(resp)+"====\n");
            if (resp === undefined) {
                callback(undefined);
                return;
            }
            ...
            console.log("==== wrapped callback final return: "+JSON.stringify(value)+"=====");
    ...
    kettle.dataSource.CouchDB.finalInit = function (that) {
        ...
        that.set = function set(directModel, model, callback) {
           console.log("==== couchDB.set called with set("+JSON.stringify(directModel || "")+", "+JSON.stringify(model || "")+", "+JSON.stringify(callback || "")+")====\n");

            model = {value: model};
            var couchDBCallback = kettle.dataSource.CouchDB.wrapCallback(callback);
            originalGet(directModel, function onSuccess(resp) {
                //if no error ocurred during creation (undefined means the value didn't already
                //exist which is acceptable as well)
               console.log("==== original Get callback successful - returned ("+JSON.stringify(resp)+"====\n");

                if (resp && !resp.error) {
                    model._id = resp._id;
                    model._rev = resp._rev;
                }
                console.log("==== originalSet("+JSON.stringify(directModel || "")+", "+JSON.stringify(model || "")+", "+JSON.stringify(callback || "")+")====\n");
