 ...
 * NODE_ENV, ii) the supplied argument {String} if any, iii) the 3rd command line argument. This value
...
kettle.config.getNodeEnv = function (outerDefault) {
    var nodeEnv = process.env.NODE_ENV || outerDefault || fluid.get(process.argv, "3");
    if (nodeEnv) {
        console.log("Selected configuration name " + nodeEnv);
    } else {
        fluid.fail("No configuration specified in either NODE_ENV, internal default, or 2nd command line argument");
    }
    return nodeEnv;
};

kettle.config.getConfigPath = function () {
    var configPath = fluid.get(process.argv, "2");
    if (!configPath) {
...
kettle.config.expectedTopLevel = fluid.arrayToHash(["type", "options", "includes", "require"]);
...
kettle.config.createDefaultsImpl = function (prefix, filePath) {
    ...
    var includes = fluid.makeArray(configFile.includes),
