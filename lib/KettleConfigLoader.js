...
kettle.config.createDefaultsImpl = function (prefix, filePath) {
    ...
    if (filePath.charAt(0) === "%") {
        fullPath = kettle.module.resolvePath(filePath);
    ...
    fluid.each(requires, function loadRequire(requireId) {
        if (requireId.charAt(0) === "%") {
            requireId = kettle.module.resolvePath(requireId);
        }
        try {
...
kettle.config.createDefaults = function (options) {
    return kettle.config.createDefaultsImpl(kettle.module.resolvePath(options.configPath), options.configName + ".json");
};
