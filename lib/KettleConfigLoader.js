...
var createDefaultsImpl = function (prefix, filePath) {
    var fullPath;
    console.log("Resolving filePath " + filePath + " with prefix " + prefix);
    ...
    var configFile;
    console.log("LOADING CONFIG AT " + fullPath);
    ...
    console.log("Created defaults " + fluid.prettyPrintJSON(configFile.options) + " for type " + configFile.typeName);
...
kettle.config.createDefaults = function (options) {
    return createDefaultsImpl(fluid.module.resolvePath(options.configPath), options.configName + ".json");
};
