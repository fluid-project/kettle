 ...
 * @param {kettle.staticRequestHandler} handler - The handler to be registered
 */
kettle.staticRequestHandler.registerHandler = function (server, handler) {
    var requestHandlerRecord = {
        type: handler.options.handlerType,
        options: handler.options.handlerOptions
    };
    kettle.staticRequestHandler.copyProps.forEach(function (prop) {
        if (handler.options[prop] !== undefined) {
            requestHandlerRecord[prop] = handler.options[prop];
        }
    });
    var handlerPath = fluid.pathForComponent(handler);
    kettle.server.registerOneHandler(server, handler, requestHandlerRecord, handlerPath[handlerPath.length - 1]);
};
