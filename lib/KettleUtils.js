 ...
 *
 * @return {Object} The parser to use (defaulting to json)
 */
kettle.getSuitableParser = function (fileName) {
    return (path.extname(fileName).toLowerCase() === ".json5") ?
        kettle.dataSource.parseJSON5 : kettle.dataSource.parseJSON;
};

kettle.readFileSync = function (fileName, message) {
    ...
    var sequence = [{
        ...
        listener: kettle.getSuitableParser(fileName)
    }];
