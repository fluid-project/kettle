...
kettle.module.dirs = fluid.getMembers(fluid.module.modules, "baseDir");

// A suitable set of terms for interpolating module root paths into dataSource URLs
kettle.module.terms = fluid.transform(kettle.module.dirs, function (dir) {
    return "noencode:" + dir;
});

kettle.module.resolvePath = function (path) {
    return fluid.stringTemplate(path, kettle.module.dirs);
};
...
fluid.defaults("kettle.dataSource.distributeDevTerms", {
    ...
    distributeOptions: {
        port: {
            record: "{kettle.server}.options.port",
            target: "{that dataSource}.options.termMap.port"
        },
        modules: {
