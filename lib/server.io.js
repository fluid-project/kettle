...
(function () {
    ...
    fluid.defaults("kettle.server.io", {
        ...
        invokers: {
            getSessionGrade: {
                funcName: "kettle.server.io.getSessionGrade",
    ...
    kettle.server.io.getSessionGrade = function (gradeNames) {
    ...
    kettle.server.io.registerHandler = function (that, handler, context) {
        ...
        ioHandler.on("connection", function (socket) {
            socket.on("message", function onMessage(data, send) {
                ...
                that.requests.events.onHandleRequest.fire(request, context);
            });
        });
    };
    ...
    kettle.server.io.registerHandlers = function (that) {
        fluid.each(that.options.handlers, function (handler, context) {
            that.events[handler.type === "io" ? "onRegisterIOHandler" :
                "onRegisterHandler"].fire(handler, context);
        });
    };
