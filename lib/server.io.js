...
(function () {
     ...
     */
    kettle.server.io.registerHandler = function (requests, io, handler, context) {
        var ioHandler = io.of(handler.route);
     ...
     */
    kettle.server.io.registerHandlers = function (that, handlers) {
        fluid.each(handlers, function register(handler, context) {
            if (!handler) {
                return;
            }
            if (handler.type === "io") {
                kettle.server.io.registerHandler(that.requests, that.io,
                    handler, context);
            } else {
